...
This Repository follows the gitflow workflow method, see [tutorial](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow):
...
2. [Firmware Setup](#firmware-setup)
    - [Global Settings](#global-settings)
        ...
        - [IM21 BLUETOOTH DOOR SENSOR SETTINGS](#im21-bluetooth-door-sensor)
...
3. [Console Function Instructions](#console-function-instructions)
    ...
    - [ChangePwd](#changepwd)
    - [getWifiLog](#getwifilog)
    - [doorSensorID](#doorsensorid)
    - [xethruConfigVals](#xethruconfigvals)
...
#### IM21 BLUETOOTH DOOR SENSOR SETTINGS
...
What I can tell you is led, noisemap, and sensitivity settings are ints, so defining a decimal number will break the code.  Min detect and max detect are floats, so decimal numbers can be used there.  An example is below:
...
Max and min Detect settings set the maximum and minimum range (in metres) for detecting movement
...
A string whose first character is an index number 0 - 3 indicating which SSID/password pair this SSID belongs to, and the remaining characters are the SSID itself.  For example, to place ClientSSID in the 0th element of the firmware’s five-SSID array, enter 0ClientSSID
A string containing a password defined in odetect_config.h.  If this password is entered, the SSIDs currently stored in flash memory will be published to the cloud.
...
A string whose first character is an index number 0 - 3 indicating which SSID/password pair this password belongs to, and the remaining characters are the password itself.  For example, if ClientSSID is in the 0th element of the firmware’s five-SSID array, enter its password as 0password_for_ClientSSID0  
A string containing a password defined in odetect_config.h.  If this password is entered, the passwords currently stored in flash memory will be published to the cloud.
...
e - echos, aka publish to cloud, the wifi log’s current int
c - clears the wifi log by setting the int = 0
...
Three byte door ID separated by colons, for example 3C:2B:1A  See Additional Information section above for where to locate an IM21 door sensor’s door ID.  
e - echos, aka publish to cloud, the door ID the Particle is currently connected to
...
String containing led, noisemap, sensitivity, min_detect, and max_detect settings, in that order, separated by commas.  
Led, noisemap, and sensitivity are ints so entering a decimal number will cause truncation and possibly an error.
Min_detect and max_detect are floats.
**Sample input:**   0, 0, 5, 0.5, 4.0
**e - echos**, aka publish to cloud, the current Xethru configuration settings
...
**Devicetype - usually “Xethru”**
**Locationid - I believe this is the client site name**
**Deviceid - Specific Xethru device ID (as there may be several at a single client site)**
**Distance -**
...
**mySSIDs[0]** - string containing this password
**mySSIDs[1]** - string containing this password
Etc up to mySSIDs[4]
**Connected to:** - string containing SSID the Particle WiFi module is currently connected to
...
**myPasswords[0]** - string containing this password
**myPasswords[1]** - string containing this password
...
Length of disconnect in seconds - integer number of seconds
...
**doorID** - three byte device ID for the IM21, broadcast by the device in its advertising data, and also contained on a sticker on the device.  See IM21 Bluetooth Door Sensor Settings section for more information re door’s device ID.
**Data** - a single byte indicating whether door is opened or closed. 0x00 = closed, 0x02 = open.
**Control** - a single byte that increments by 1 every time the door sensor advertises data. Useful to count events (for battery usage calculations) and to ensure Particle does not publish duplicate events.
...
**doorID** - three byte device ID for the IM21, broadcast by the device in its advertising data, and also contained on a sticker on the device.  See [IM21 Bluetooth Door Sensor Settings](#im21-bluetooth-door-sensor-settings) section for more information re door’s device ID.
**Data** - a single byte indicating whether door is opened or closed. 0x00 = closed, 0x02 = open.
**Control** - a single byte that increments by 1 every time the door sensor advertises data. Useful to count events (for battery usage calculations) and to ensure Particle does not publish duplicate events.
**Warning** - text string saying “Missed a door event, OR new doorID, OR Particle was rebooted.”
