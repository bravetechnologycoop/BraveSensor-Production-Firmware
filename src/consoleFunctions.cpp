...
void setupConsoleFunctions(){
  ...
  Particle.function("Change_XeThru_Slow_Threshold", slow_threshold_set);
...
int slow_threshold_set(String input){
  ...
  //else parse new threshold
    ...
    if(threshold == 0){
      ...
      EEPROM.put(ADDR_SLOW_THRESHOLD, threshold);
